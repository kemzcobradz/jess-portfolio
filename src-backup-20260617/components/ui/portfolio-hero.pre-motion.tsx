      ...
      <section id="home" className="relative min-h-screen flex items-center justify-center px-6 md:px-12 lg:px-20 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto w-full grid md:grid-cols-2 gap-12 items-center relative z-10">
          ...
          <div className="flex justify-center md:justify-end">
            <div className="relative">
              {/* Glow behind image */}
              <div className="absolute -inset-4 bg-[#C3E41D]/10 rounded-3xl blur-2xl" />
              <div className="relative w-64 h-80 sm:w-80 sm:h-96 md:w-[380px] md:h-[460px] rounded-2xl overflow-hidden border border-white/10">
                <img src="/headshot.png" alt="Jess Cobrado, SEO Virtual Assistant and Executive Assistant" className="w-full h-full object-cover object-top" style={{ objectPosition: "50% 15%" }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-6 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide" style={{ scrollbarWidth: "none" }}>
          {highlights.map((item, i) => (
            <Section key={item.title} delay={i * 100}>
              <div
                className={`min-w-[300px] sm:min-w-[350px] snap-center bg-[#111] border rounded-xl p-6 transition-all duration-500 cursor-pointer ${
                  activeHighlight === i
                    ? "border-[#C3E41D]/40 shadow-[0_0_30px_rgba(195,228,29,0.08)]"
                    : "border-white/5 hover:border-[#C3E41D]/20"
                }`}
        ...
        <div className="flex justify-center gap-2 mt-6">
          {highlights.map((_, i) => (
            <button
              key={i}
              type="button"
              className={`w-2 h-2 rounded-full transition-colors duration-300 cursor-pointer ${activeHighlight === i ? "bg-[#C3E41D]" : "bg-neutral-700"}`}
              onClick={() => setActiveHighlight(i)}
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {services.map((service, i) => (
            <Section key={service.title} delay={i * 100}>
              <div className="group bg-[#111] border border-white/5 rounded-xl p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(195,228,29,0.06)] hover:border-[#C3E41D]/30 h-full">
                <h3 className="text-xl font-semibold text-white mb-6 group-hover:text-[#C3E41D] transition-colors duration-300">{service.title}</h3>
        ...
        <div className="space-y-8">
          {projects.map((project, i) => (
            <Section key={project.title} delay={i * 100}>
              <div className="group bg-[#111] border border-white/5 rounded-xl p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(195,228,29,0.06)] hover:border-[#C3E41D]/20">
      ...
      <section className="py-24 px-6 max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {otherProjects.map((project, i) => (
            <Section key={project.title} delay={i * 100}>
              <div className="group bg-[#111] border border-white/5 rounded-xl p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(195,228,29,0.06)] hover:border-[#C3E41D]/20 h-full">
