...
const NeuralVortexBg = () => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        color = vec3(1.0, 0.92, 0.66);
        color = mix(color, vec3(0.77, 0.89, 0.11), 0.32 + 0.16 * sin(2.0 * u_scroll_progress + 1.2));
        color += vec3(0.2, 0.18, 0.0) * sin(2.0 * u_scroll_progress + 1.5);
        color = color * noise;
  ...
  return (
    <canvas 
      ref={canvasRef} 
      className="fixed inset-0 w-full h-full pointer-events-none z-0 opacity-40 blur-[2px]"
    />
  );
};
